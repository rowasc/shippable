...
plan all work without an Anthropic key), and its outbound Anthropic + GitHub
calls are pointed at a local **fake upstream** — so the full
browser→vite→server→upstream path runs for real, including the server's own
request building, response parsing, and error mapping. Tests `page.route()`
only the one genuinely external browser fetch (the `.diff` URL in J4) and
...
The `webServer` block boots three processes:
...
- the real `server/` (`npm start`) on **:3001**, which vite proxies
  ...
  be on the allowlist or worktree/auth writes come back 403. It's also
  given `ANTHROPIC_BASE_URL` and `SHIPPABLE_GITHUB_API_BASE` pointing at
  the fake upstream.
- the **fake upstream** (`e2e/scripts/fake-upstream.mjs`) on **:3002** —
  stands in for the Anthropic (`POST /v1/messages`, JSON + SSE) and GitHub
  (`GET /repos/...`) APIs. The `SHIPPABLE_GITHUB_API_BASE` override is a
  test-only seam in `server/src/github/url.ts`.
...
| `scripts/fake-upstream.mjs` | fake Anthropic + GitHub upstream the real server talks to | — |
...
6. AI + GitHub paths go through the real server to the fake upstream. The
   default `visit()` still mocks `/api/auth/list` for determinism — tests
   that need the *real* auth store (storing a key/token, then exercising
   `/api/plan` or `/api/github/pr/load`) must `page.unroute("**/api/auth/
   list")` first. Extend `scripts/fake-upstream.mjs` for new responses.
