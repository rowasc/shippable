...
export default storyboard({
  ...
  steps: [
    ...
    waitFor(".plan"),
    wait(300),
    shot("plan_idle", 0.9, { caption: "Open the diff — a rule-based plan from the file structure" }),
    wait(1100),
    shot("plan_idle_hold", 3.0),
    ...
    shot("plan_loading", 0.8, { caption: "Sending the diff to Claude for review" }),
    wait(900),
    shot("plan_loading_hold", 2.1),

    // 3. Wait for the AI plan to swap in — the loading status disappears
    //    and the intent claims re-render with new text.
    wait(1500),
    shot("plan_ai_arrived", 0.8, { caption: "Claude's plan: intent claims grounded in the code" }),
    wait(900),
    shot("plan_ai_hold", 3.0),

    // 3. Pause on the entry-points section so the reader can see the
    //    "where to start" picks.
    wait(800),
    shot("plan_entries", 2.4, { caption: "Recommended entry points — where to start reading" }),

    // 4. Click the first entry point — overlay closes, cursor lands on the
    //    defining hunk of storage.ts.
    press("Enter", { hold: 500 }),
    shot("jumped_to_entry", 0.9, { caption: "Jump straight to the defining hunk" }),
    wait(900),
    shot("jumped_to_entry_hold", 2.7),

    // 5. Reopen plan with `p` to show the AI plan is sticky for this session.
    press("p", { hold: 350 }),
    shot("plan_reopened", 0.8, { caption: "Press p to revisit the plan anytime" }),
    wait(900),
    shot("plan_reopened_hold", 2.4),
  ],
});
