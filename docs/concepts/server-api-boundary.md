...
- **Credentials.** `POST /api/auth/{set, has, clear}` and `GET /api/auth/list`. Generic credential surface. Body shape: `{ credential: { kind: "anthropic" } | { kind: "github", host: string }, value?: string }`. `list` returns `{ credentials: Credential[] }` — identifier-only, never values. The server-side store is in-memory; the web app rehydrates it from the Tauri Keychain at boot. Replaced the older `/api/github/auth/*` endpoints.
