...
When you open a ChangeSet, the plan overlay tells you what the diff is doing and where to start reading. The headline and the file/symbol map are computed from the diff itself — no model involved. The intent claims and the entry points are generated by an AI, but only when the reviewer asks for it: a "Send to Claude" button sits in the header on first open, and the diff doesn't leave your machine until you click it.
