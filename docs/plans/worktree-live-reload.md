...
- Block-comment keys preserve their span size when re-anchored: the new `lo` is the matched window centre, `hi` becomes `lo + originalSpan`, clamped to the new hunk.
