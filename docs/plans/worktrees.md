...
- **Reverse direction = per-worktree queue + three-event hook.** Reviewer's per-thread replies and freeform messages land on a per-worktree, in-memory queue on the local server; a documented hook fires on `UserPromptSubmit`, `PostToolUse`, and `SessionStart`, pulls the queue, and grafts the payload onto the agent's turn as `additionalContext`. We label this honestly as "delivers on the agent's next tool call or session start" — no mid-turn interrupt claim. Mid-turn interrupt stays speculative, behind a future "live mode" toggle. See `docs/plans/push-review-comments.md`.
- **Slice (d)'s file-based approach was superseded.** The original sketch used `<worktree>/.shippable/inbox.md` plus a `.shippable/` line auto-appended to the shared `$(git rev-parse --git-common-dir)/info/exclude` so the file stayed git-ignored without dirtying the tracked `.gitignore`. That mechanism shipped first and worked, but the queue-and-hook channel from `push-review-comments.md` subsumed it cleanly: no on-disk state in the worktree, atomic per-pull semantics, multi-comment batches, delivered-history surface, and a single delivery signal that drives both the freeform composer and the per-thread Send button. Slice (e) of `push-review-comments.md` retired the file-based path entirely.
