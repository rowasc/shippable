...
**(b) Per-worktree commit picker. ✅ Shipped.** HEAD is fine for "what just landed" but not for "let me review the last three commits." A `RangePicker` lives on each worktree row in `LoadModal` ("pick range…") and on the topbar (`⇄ range`) once a worktree changeset is loaded — click sets `from`, shift-click sets `to`, "include uncommitted changes" is enabled only when `to = HEAD`, and a per-row "just this" shortcut loads a single commit. Server contract in `rangeChangeset` / `listCommits`. See [commit-range-picker](../features/commit-range-picker.md) for the user-facing surface.
