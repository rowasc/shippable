...
6. **Worktree file sign-offs don't survive a reload (J2).** Repro: load a worktree, sign off a file (`Shift+M`), reopen the app at `/`. The session resumes — cursor and read marks come back — but the sign-off is gone. The sign-off *is* persisted (it reaches `localStorage["shippable:review:v1"]`), but on resume it's dropped by `loadSession`'s `validFileIds` filter (`persist.ts:272`): the persisted `reviewedFiles` ids no longer match the re-derived worktree changeset's file ids. Read marks survive the parallel `validHunkIds` filter, so the changeset id itself is stable — the mismatch is narrower than a whole-id change and isn't fully pinned. Paste/URL and fixture changesets are unaffected (J4's sign-off-persist passes). `web/e2e/journey-2-worktree.spec.ts` deliberately asserts read-mark persistence, not sign-off, because of this.
